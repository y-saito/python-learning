...
## ファイル名の命名ルール
- 日時を含める必要があるドキュメントは `report_YYYYMMDD_HHMMSS_<作業内容の簡単な説明文>.md` の形式で `docs/` 配下に保存する。
...
## コメント
- コードには適切なコメントを付けること。特に、複雑なロジックや重要な処理には詳細な説明を加える。
- 日本語でコメントを記述することが望ましいが、必要に応じて英語も使用することができる。